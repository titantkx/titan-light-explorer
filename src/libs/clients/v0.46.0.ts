...
function proposalAdapter(p: any): GovProposal {
  if (p) {
    ...
    p.final_tally_result = {
      yes_count: p.final_tally_result?.yes_count,
      no_count: p.final_tally_result?.no_count,
      no_with_veto_count: p.final_tally_result?.no_with_veto_count,
      abstain_count: p.final_tally_result?.abstain_count,
    };
  }
  return p;
}
