...
import { getDenomTraces, getStakingParam } from '../../../libs/utils/http';
...
function updateIBCToken() {
  ...
  if (ibcDenomTraces.value[denom.value]) {
    ...
    getDenomTraces(props.endpoint, hash).then((trace: any) => {
...
const units = computed(() => {
  ...
  const list = props.metadata[denom.value].denom_units.sort(
    (a: any, b: any) => b.exponent - a.exponent
  );
...
function initial() {
  ...
  getStakingParam(props.endpoint).then((x: any) => {
    ...
    <div class="form-control">
      <label class="label">
        ...
        <span v-if="sourceChain" class="text-xs"
          >{{ sourceChain.channel_id }}/{{ sourceChain.port_id }}</span
        >
      </label>
      <select
        v-model="dest"
